        ...
        <div className='border border-blue-300 rounded-2xl p-2 bg-blue-100'>
          <div className="grid gap-3 mb-4">
            ...
            {cartItems.map((item, index) => (
              <p key={index} className='bg-blue-200 px-2 py-1 rounded-full text-sm'>
                {item.name} ({item.quantity}),
              </p>
              ))}
            </div>
            <div className='text-center'>
              <p className="inline-block bg-blue-300 px-4 py-1 rounded-full font-semibold text-lg">
          ...
          <form onSubmit={handleSubmit} className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div className="flex flex-col">
              ...
              <select className='border border-gray-400 rounded-lg p-1 text-gray-700' value={formData.payment} name='payment' onChange={handleChange}>
                <option value="cash on delivery" defaultValue>
                  Cash on Delivery
                </option>
              </select>
            </div>
